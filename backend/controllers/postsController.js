...
-------------------------------------------------*/
module.exports.createPostCtl = asyncHandler(async (req, res) => {
    // 1. Validation for image
    if(!req.file) {
        return res.status(400).json({ message: "No image provided, Please Upload Image" });
    };
...
module.exports.updatePostCtrl = asyncHandler(async (req,res) => {
    ...
    // 2. Get the post from DB, Check if post exist
    const post = await Post.findById(req.params.id);
    if(!post) {
 ...
 *  @route    /api/posts/upload-iamge/:id
 *  @method   PUT
 *  @access   private (only owner of the post)
-------------------------------------------------*/
